...
func NewHTTPHandler(router *mux.Router, endpoints Set, logger log.Logger) http.Handler {
	...
	options := []httptransport.ServerOption{
		...
		httptransport.ServerBefore(GetBearerTokenFromHeaderToContext),
		//zipkinServer,
	}
...
func GetBearerTokenFromHeaderToContext(ctx context.Context, r *http.Request) context.Context {
	authHeader := r.Header.Get("Authorization")
	token := strings.Replace(authHeader, "Bearer ", "", 1)
	return context.WithValue(ctx, APIToken, token)
}
...
// decodeHTTPStatImportRequest is a transport/http.DecodeRequestFunc that decodes a
